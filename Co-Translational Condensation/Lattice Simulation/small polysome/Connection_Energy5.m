...
% Seq5 Cross-polymer
interaction5=[0 0 0; 0 -50 -450; 0 -450 -50];
...
function  energy_list = energy_list(protein_list)
    ...
    for i = 1: length(branch_start)
        energy_list(branch_start(i):branch_start(i)+3*i-1) = flip(protein_list(1:3*i)); % Reverse the order of selection, because N terminal should be near the end
    end
