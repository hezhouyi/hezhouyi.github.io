...
function  energy_list = energy_list(protein_list)
    ...
    for i = 1: length(branch_start)
        energy_list(branch_start(i):branch_start(i)+2*i-2) = flip(protein_list(1:2*i-1)); % Reverse the order of selection, because N terminal should be near the end
    end
